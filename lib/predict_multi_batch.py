...
def batch_predict(data,arch_cnn,arch_caps,model_cnn,model_caps,nclass,outputfile,foldnum,num_ptms,ptmtype,nclass_init=None):
...
def cal_musit(inputfile ,output, model_prefix ):
    
    ensure_dir(output) #mkfolders for outputfile
    # websiteoutput = open(output+"_predicted_num.txt",'w') #generate this file at the beginning! 
    ...
    if model_prefix is None:
      ...
      foldername,filename = os.path.split(model_prefix)
      print(foldername)
      print(filename)
      ...
      for ptmtype in modeloptins:
        ...
        testfrag,ids,poses,focuses,idlist=extractFragforMultipredict(inputfile,window,'-',focus=residues)
        foldnum=10
        predictproba,y_true=batch_predict(testfrag,arch_cnn,arch_caps,model_cnn,model_caps,nclass,output,foldnum,num_ptms,ptmtype)           
        poses=poses+1;
        ptmindex+=1
      ...
      target = open(output+"_results.txt", "w")
      #add a header
      ...
      target.close()
    #   websiteoutput = open(output+"_predicted_num.txt",'w')
